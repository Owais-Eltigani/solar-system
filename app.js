...
const planetModel =
  mongoose.models.planets || mongoose.model("planets", dataSchema);
...
const PLANETS_SEED = [
  ...
  },
  {
    ...
    id: 7,
    description:
      "Uranus rotates on its side and is an ice giant with a pale blue color.",
  ...
  },
  {
    ...
    id: 8,
    description:
      "Neptune is the farthest known planet from the Sun and has strong winds.",
...
function ensurePlanetsSeedData() {
  planetModel.find({}, { id: 1 }, function (findError, existingPlanets) {
    ...
    const existingIds = new Set(
      existingPlanets.map(function (planet) {
        return planet.id;
      }),
    );
